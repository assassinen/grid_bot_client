...
class TinkoffExchangeInterface:
    def __init__(self, key, secret, base_url, api_url, instrument):
        ...
        self.driver = webdriver.Remote(
            ...
            desired_capabilities={"browserName": "chrome", "enableVNC": True},
        )
        ...
        try:
            if metod == "GET":
                response = requests.get(
                    url=url, headers=headers, json=data, params=params
                )
            if metod == "POST":
    ...
    def get_new_session_id(self):
        ...
        self.session_id = [
            cookie.get("value") for cookie in cookies if cookie.get("name") == "psid"
        ]
    ...
    def get_session_id(self):
        with open(self.session_id_file) as f:
            data = json.load(f)
            if (
                data.get("time") + self.ttl_session_id > int(time.time())
                and data.get("time") > self.srart_time
            ):
