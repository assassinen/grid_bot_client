...
class BitfinexExchangeInterface:
    ...
    def generate_auth_headers(self, path, body):
        ...
        h = hmac.new(
            self.secret.encode("utf8"), signature.encode("utf8"), hashlib.sha384
        )
        signature = h.hexdigest()
        return {"bfx-nonce": nonce, "bfx-apikey": self.key, "bfx-signature": signature}
    ...
    def get_positions(self):
        ...
        wallet = [
            wallet
            for wallet in self._post(endpoint)
            if wallet[0] == "exchange" and wallet[1] == "BTC"
        ]
    ...
    def get_open_orders(self):
        ...
        return [
            self.get_order_params_from_responce(order)
            for order in open_orders
            if order[8] == "EXCHANGE LIMIT"
        ]
    ...
    def get_trades(self, last_trade_time):
        endpoint = f"auth/r/trades/{self.instrument}/hist"
        params = {"start": last_trade_time, "limit": 2000}
        trades = self._post(endpoint, params)
        return [
            self.get_trade_params_from_responce(trade)
            for trade in trades
            if trade[6] == "EXCHANGE LIMIT"
        ]
    ...
    def get_order_state(self, order_id):
        ...
        return (
            self.get_order_params_from_responce(order[0])
            if len(order) > 0
    ...
    def create_order(self, order):
        ...
        if "SUCCESS" in result:
            return [
                self.get_order_params_from_responce(orders) for orders in result[4]
            ][0]
