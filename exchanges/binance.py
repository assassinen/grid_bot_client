...
class BinanceExchangeVanillaOptionsInterface(BinanceExchangeBaseInterface):
    ...
    def get_open_orders(self):
        ...
        open_orders = [
            i for i in result.get("data") if i.get("symbol") == self.instrument
        ]
...
class BinanceExchangeCoinFuturesInterface(BinanceExchangeBaseInterface):
    def get_positions(self):
        path = "positionRisk"
        result = self._request(
            "get", f"{self.uri}/{path}", signed=True, force_params=True, data={}
        )
        position = [i for i in result if i.get("symbol") == self.instrument]
        position = position[0] if len(position) > 0 else {}
        return {
            "average_price": float(position.get("entryPrice")),
    ...
    def get_order_state(self, order_id):
        try:
            path = "order"
            params = {"symbol": self.instrument, "orderId": order_id}
            order = self._request(
                "get", f"{self.uri}/{path}", signed=True, force_params=True, data=params
            )
        except Exception as r:
    ...
    def get_order_params_from_responce(self, responce):
        ...
        return {
            "price": float(responce.get("price")),
            ...
            "status": status_mapp.get(
                responce.get("status"), responce.get("status").lower()
            ),
    ...
    def create_order(self, order=""):
        ...
        order = self._request(
            "post", f"{self.uri}/{path}", signed=True, force_params=True, data=params
        )
        return self.get_order_params_from_responce(order)
    ...
    def cancel_all_orders(self):
        ...
        return self._request(
            "delete", f"{self.uri}/{path}", signed=True, force_params=True, data=params
        )
